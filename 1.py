...
def recog_numplate():
    ...
    for i, plate_img in enumerate(plate_imgs):
        ...
        has_digit = False
        for c in chars:
            if ord('∞°') <= ord(c) <= ord('∆R') or c.isdigit():
                if c.isdigit():
                    has_digit = True
                result_chars += c    
        print(result_chars)
...
def send_data():
    ...
    db = client.mydb #mydb¥¬ µ•¿Ã≈Õ∫£¿ÃΩ∫ ¿Ã∏ß
    doc = {'name':'amy','age':25} # µ•¿Ã≈Õ «œ≥™øπΩ√
    db.users.insert_one(doc) # 'users' ¿Ã∏ß¿« collection¿Ã ª˝º∫µ«∞Ì, 'users' collectionø° doc µÒº≈≥ ∏Æ∞° ¿˙¿Â.
